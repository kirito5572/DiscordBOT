...
public class VersionCommand implements ICommand {
    ...
    public void handle(List<String> args, GuildMessageReceivedEvent event) {
        if(App.isDEBUG_MODE() || App.isONLINE_DEBUG()) {
            event.getChannel().sendMessage("빌드 버젼 V 1.4.1 Preview 4(" + App.getTime() + ")").queue();
            return;
        }
        event.getChannel().sendMessage("빌드 버젼 V 1.4.1 Preview 3 (" + App.getTime() + ")").queue();
        if(event.getGuild().getId().equals("508913681279483913")) {
            ...
            event.getChannel().sendMessage("업데이트 내역: \n" +
                    "```" +
                    " - 서버 커스텀 명령어 서버밴 시간 인수에 y(Year) 추가\n" +
                    " - 필터링 데이터베이스 추가" +
                    "```").queue();
        }
    }
