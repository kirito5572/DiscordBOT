    ...
    public void handle(List<String> args, @NotNull EventPackage event) {
        ...
        event.getGuild().retrieveCommands().queue(commands -> {
            ...
            for(SlashData slashData : commandDataList) {
                ...
                if (!noRefresh) {
                    if (!needUpdate) {
                        event.getGuild().upsertCommand(slashData.invoke, slashData.description).queue(command ->
                                event.textChannel().sendMessage("명령어 **" + slashData.invoke + "** 가 추가됨").queue());
                    } else {
                        event.getGuild().upsertCommand(slashData.invoke, slashData.description).queue(command ->
                                event.textChannel().sendMessage("명령어 **" + slashData.invoke + "** 가 갱신됨").queue());
                    }
                }
            }

            if (!nothing) {
                event.textChannel().sendMessage(event.member().getAsMention() + ", 모든 명령어 처리가 완료되었습니다.").queue();
            } else {
                event.textChannel().sendMessage(event.member().getAsMention() + ", 신규 또는 갱신할 명령어가 없습니다.").queue();
            }
        });
